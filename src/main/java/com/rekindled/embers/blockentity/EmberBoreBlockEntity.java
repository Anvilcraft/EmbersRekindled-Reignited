...
public class EmberBoreBlockEntity extends BlockEntity implements ISoundController, IMechanicallyPowered, IExtraDialInformation, IExtraCapabilityInformation, IUpgradeable {
	...
	public AABB getRenderBoundingBox() {
		return new AABB(worldPosition.offset(-1, -2, -1)).expandTowards(3, 2, 3);
	}

	public AABB getBladeBoundingBox() {
		return new AABB(worldPosition.offset(-1, -2, -1)).expandTowards(2, 1, 2);
	}
	...
	public boolean canMine() {
		if (canMine == null) {
			...
			if (biome != null) {
				...
				List<IBoringRecipe> recipes = new ArrayList<>(level.getRecipeManager().getRecipesFor(RegistryManager.BORING.get(), context, level).stream().map(net.minecraft.world.item.crafting.RecipeHolder::value).toList());
	...
	public class EmberBoreInventory extends ItemStackHandler {
		...
		public ItemStack insertItem(int slot, ItemStack stack, boolean simulate) {
			...
			if (currentFuel.isEmpty()) {
				int burnTime = stack.getBurnTime(RegistryManager.BORING.get());
				if (burnTime != 0)
