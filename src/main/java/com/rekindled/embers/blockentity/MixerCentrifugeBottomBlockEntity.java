...
public class MixerCentrifugeBottomBlockEntity extends BlockEntity implements IMechanicallyPowered, ISoundController, IExtraDialInformation, IExtraCapabilityInformation {
	...
	public void loadAdditional(CompoundTag nbt, HolderLookup.Provider registries) {
		...
		north.readFromNBT(registries, nbt.getCompound("northTank"));
		south.readFromNBT(registries, nbt.getCompound("southTank"));
	...
	public void saveAdditional(CompoundTag nbt, HolderLookup.Provider registries) {
		...
		nbt.put("northTank", north.writeToNBT(registries, new CompoundTag()));
		nbt.put("southTank", south.writeToNBT(registries, new CompoundTag()));
	...
	public void addDialInformation(Direction facing, List<Component> information, String dialType) {
		if (FluidDialBlock.DIAL_TYPE.equals(dialType)) {
			...
			if (north.getFluid().getFluid().defaultFluidState().is(EmbersFluidTags.INGOT_TOOLTIP) && north.getFluid().getAmount() >= FluidAmounts.nuggetValue())
			...
			if (east.getFluid().getFluid().defaultFluidState().is(EmbersFluidTags.INGOT_TOOLTIP) && east.getFluid().getAmount() >= FluidAmounts.nuggetValue())
			...
			if (south.getFluid().getFluid().defaultFluidState().is(EmbersFluidTags.INGOT_TOOLTIP) && south.getFluid().getAmount() >= FluidAmounts.nuggetValue())
			...
			if (west.getFluid().getFluid().defaultFluidState().is(EmbersFluidTags.INGOT_TOOLTIP) && west.getFluid().getAmount() >= FluidAmounts.nuggetValue())
