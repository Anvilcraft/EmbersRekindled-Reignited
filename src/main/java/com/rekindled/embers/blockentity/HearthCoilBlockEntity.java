...
public class HearthCoilBlockEntity extends BlockEntity implements ISoundController, IExtraDialInformation, IExtraCapabilityInformation, IUpgradeable {
	...
	public static <T extends AbstractCookingRecipe> void serverTick(Level level, BlockPos pos, BlockState state, HearthCoilBlockEntity blockEntity) {
		...
		blockEntity.upgrades = getHearthCoilUpgrades(level, pos);
		...
		int superheatingBlazeBurners = heatedBlazeBurners > 0
				...
				: 0;
		double blazeBurnerCost = superheatingBlazeBurners * ConfigManager.CREATE_BLAZE_BURNER_SUPERHEAT_COST.get()
				+ (heatedBlazeBurners - superheatingBlazeBurners) * ConfigManager.CREATE_BLAZE_BURNER_EMBER_COST.get();
		double emberCost = UpgradeUtil.getTotalEmberConsumption(blockEntity, ConfigManager.HEARTH_COIL_EMBER_COST.get() + blazeBurnerCost, blockEntity.upgrades);
		...
		if (blockEntity.heat > 0 && blockEntity.ticksExisted % cookTime == 0) {
			...
			if (!cancel) {
				...
				if (items.size() > 0) {
					...
					net.minecraft.world.item.crafting.SingleRecipeInput wrapper = new net.minecraft.world.item.crafting.SingleRecipeInput(entityItem.getItem());
	...
	public static List<UpgradeContext> getUndersideUpgrades(Level level, BlockPos pos) {
		return getHearthCoilUpgrades(level, pos);
	}

	public static List<UpgradeContext> getHearthCoilUpgrades(Level level, BlockPos pos) {
		ArrayList<UpgradeContext> upgrades = new ArrayList<>();
		for (int x = -1; x <= 1; x++) {
			for (int z = -1; z <= 1; z++) {
				UpgradeUtil.getUpgrades(level, pos.offset(x, 0, z), Direction.values(), upgrades);
			}
		}
		return upgrades;
	}
