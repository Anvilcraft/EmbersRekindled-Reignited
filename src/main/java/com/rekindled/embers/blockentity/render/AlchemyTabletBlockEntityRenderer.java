...
public class AlchemyTabletBlockEntityRenderer implements BlockEntityRenderer<AlchemyTabletBlockEntity> {
	...
	//public ResourceLocation texture = ResourceLocation.parse(Embers.MODID + ":textures/entity/alchemy_circle.png");
	...
	public void render(AlchemyTabletBlockEntity blockEntity, float partialTick, PoseStack poseStack, MultiBufferSource bufferSource, int packedLight, int packedOverlay) {
		if (blockEntity != null) {
			...
			if (!blockEntity.inventory.getStackInSlot(0).isEmpty()) {
				...
				float f2 = com.rekindled.embers.util.RenderUtil.getGroundTransformYScale(bakedmodel);
...
/*
			if (blockEntity.process != 0) {
				...
				for (float i = 0; i < 8; i ++) {
				bufferbuilder.addVertex(matrix4f, -1, 0, -1).setUv(0, 0).setColor(r, g, b, a).setUv2(lightx, lighty);
				bufferbuilder.addVertex(matrix4f, -1, 0, 1).setUv(0, 1).setColor(r, g, b, a).setUv2(lightx, lighty);
				bufferbuilder.addVertex(matrix4f, 1, 0, 1).setUv(1, 1).setColor(r, g, b, a).setUv2(lightx, lighty);
				bufferbuilder.addVertex(matrix4f, 1, 0, -1).setUv(1, 0).setColor(r, g, b, a).setUv2(lightx, lighty);
