	...
	public void render(CrystalCellBlockEntity blockEntity, float partialTick, PoseStack poseStack, MultiBufferSource bufferSource, int packedLight, int packedOverlay) {
		...
		for (float j = 0; j < 12; j++) {
			...
			for (int i = 0; i < widths.length - 1; i++) {
				...
				buffer.addVertex(matrix4f, -width, layerHeight * i - height / 2.0f, -width).setUv(0, 0).setColor(1, 1, 1, 0.65f);
				buffer.addVertex(matrix4f, width, layerHeight * i - height / 2.0f, -width).setUv(0.5f, 0).setColor(1, 1, 1, 0.65f);
				buffer.addVertex(matrix4f, nextWidth, layerHeight + layerHeight * i - height / 2.0f, -nextWidth).setUv(0.5f, 0.5f).setColor(1, 1, 1, 0.65f);
				buffer.addVertex(matrix4f, -nextWidth, layerHeight + layerHeight * i - height / 2.0f, -nextWidth).setUv(0, 0.5f).setColor(1, 1, 1, 0.65f);
