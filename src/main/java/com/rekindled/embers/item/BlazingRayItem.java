...
public class BlazingRayItem extends Item implements IProjectileWeapon {
	...
	public void releaseUsing(ItemStack stack, Level level, LivingEntity entity, int timeLeft) {
		if (!level.isClientSide) {
			double charge = (Math.min(ConfigManager.BLAZING_RAY_MAX_CHARGE.get(), 72000 - timeLeft)) / (double) ConfigManager.BLAZING_RAY_MAX_CHARGE.get();
	...
	public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand hand) {
		...
		if (!ItemData.hasTag(stack) || ItemData.getTag(stack).getLong("lastUse") + ConfigManager.BLAZING_RAY_COOLDOWN.get() <= level.getGameTime() || player.isCreative()) {
