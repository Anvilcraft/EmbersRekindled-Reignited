...
public class CinderStaffItem extends Item implements IProjectileWeapon {
	...
	public void releaseUsing(ItemStack stack, Level level, LivingEntity entity, int timeLeft) {
		if (!level.isClientSide) {
			double charge = (Math.min(ConfigManager.CINDER_STAFF_MAX_CHARGE.get(), 72000 - timeLeft)) / (double) ConfigManager.CINDER_STAFF_MAX_CHARGE.get();
		...
		ItemData.getOrCreateTag(stack).putLong("lastUse", level.getGameTime());
	...
	public void onUseTick(Level level, LivingEntity player, ItemStack stack, int count) {
		if (ItemData.hasTag(stack) && ItemData.getTag(stack).getLong("lastUse") + ConfigManager.CINDER_STAFF_COOLDOWN.get() > level.getGameTime() && !(player instanceof Player && ((Player) player).isCreative()))
			player.stopUsingItem();
		double charge = (Math.min(ConfigManager.CINDER_STAFF_MAX_CHARGE.get(), 72000 - count)) / (double) ConfigManager.CINDER_STAFF_MAX_CHARGE.get();
	...
	public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand hand) {
		...
		if (!ItemData.hasTag(stack) || ItemData.getTag(stack).getLong("lastUse") + ConfigManager.CINDER_STAFF_COOLDOWN.get() <= level.getGameTime() || player.isCreative()) {
