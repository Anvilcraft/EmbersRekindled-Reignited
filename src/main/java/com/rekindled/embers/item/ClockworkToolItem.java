...
public class ClockworkToolItem extends DiggerItem implements IEmberChargedTool {
	...
	public boolean hurtEnemy(ItemStack stack, LivingEntity target, LivingEntity attacker) {
		ItemData.getOrCreateTag(stack).putBoolean("didUse", true);
	...
	public void inventoryTick(ItemStack stack, Level world, Entity entity, int slot, boolean selected) {
		...
		if (!ItemData.hasTag(stack)) {
			ItemData.getOrCreateTag(stack);
			ItemData.getTag(stack).putBoolean("poweredOn", false);
			ItemData.getTag(stack).putBoolean("didUse", false);
		} else {
			if (entity instanceof Player player) {
				if (world.getGameTime() % 5 == 0) {
					if (EmberInventoryUtil.getEmberTotal(player) > 5.0) {
						if (!ItemData.getTag(stack).getBoolean("poweredOn")) {
							ItemData.getTag(stack).putBoolean("poweredOn", true);
						}
					} else {
						if (ItemData.getTag(stack).getBoolean("poweredOn")) {
							ItemData.getTag(stack).putBoolean("poweredOn", false);
