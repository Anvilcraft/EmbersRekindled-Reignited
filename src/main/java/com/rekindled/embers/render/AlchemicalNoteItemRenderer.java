...
public class AlchemicalNoteItemRenderer extends BlockEntityWithoutLevelRenderer {
	...
	public void renderByItem(ItemStack stack, ItemDisplayContext displayContext, PoseStack poseStack, MultiBufferSource buffer, int packedLight, int packedOverlay) {
		if (displayContext == ItemDisplayContext.FIRST_PERSON_RIGHT_HAND || displayContext == ItemDisplayContext.FIRST_PERSON_LEFT_HAND) {
			...
			float pitch = Mth.lerp(minecraft.getTimer().getGameTimeDeltaPartialTick(true), minecraft.player.xRotO, minecraft.player.getXRot());
			float swingProgress = interactionhand == InteractionHand.MAIN_HAND ? minecraft.player.getAttackAnim(minecraft.getTimer().getGameTimeDeltaPartialTick(true)) : 0.0F;
