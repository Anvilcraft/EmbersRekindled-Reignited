...
public class BoringRecipe implements IBoringRecipe {
	...
	public boolean matches(BoringContext context, Level pLevel) {
		...
		if (!biomes.isEmpty() && !biomes.contains(context.biome))
			return false;
		if (requiredBlock != null && amountRequired > 0) {
	...
	@Override
	public ItemStack getResultItem(HolderLookup.Provider registry) {
	...
	public static class Serializer extends LegacyRecipeSerializer<BoringRecipe> {

		@Override
		public BoringRecipe fromJson(ResourceLocation recipeId, JsonObject json) {
			ItemStack stack = RecipeSerialization.readItem(GsonHelper.getAsJsonObject(json, "output"));
			...
			if (blockJson != null) {
				requiredBlock = ResourceLocation.parse(blockJson.get("block_tag").getAsString());
				amountRequired = GsonHelper.getAsInt(blockJson, "amount", 0);
			}

			return new BoringRecipe(recipeId, new WeightedItemStack(stack, weight), minHeight, maxHeight, dimensions, biomes, requiredBlock == null ? null : BlockTags.create(requiredBlock), amountRequired, chance);
		}

		@Override
		public @Nullable BoringRecipe fromNetwork(ResourceLocation recipeId, FriendlyByteBuf buffer) {
			ItemStack stack = RecipeSerialization.readItem(buffer);
			...
			ResourceLocation requiredBlock = buffer.readNullable(FriendlyByteBuf::readResourceLocation);
		...
		public void toNetwork(FriendlyByteBuf buffer, BoringRecipe recipe) {
			RecipeSerialization.writeItem(buffer, recipe.result.getStack());
			...
			buffer.writeNullable(recipe.requiredBlock == null ? null : recipe.requiredBlock.location(), FriendlyByteBuf::writeResourceLocation);
