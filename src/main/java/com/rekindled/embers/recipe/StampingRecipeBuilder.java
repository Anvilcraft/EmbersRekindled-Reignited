	...
	public static class Finished implements FinishedRecipe {
		...
		public void serializeRecipeData(JsonObject json) {
			...
			if (recipe.output.right().isPresent()) {
				...
				outputJson = RecipeSerialization.saveItem(recipe.output.left().get()).getAsJsonObject();
			}
			json.add("output", outputJson);

			json.add("stamp", RecipeSerialization.saveIngredient(recipe.stamp));
			if (!recipe.input.isEmpty())
				json.add("input", RecipeSerialization.saveIngredient(recipe.input));
